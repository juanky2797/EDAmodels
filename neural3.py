...
def perform_kfold_validation(signals, tasks, userIDs, classifier, feature_selection, max_num_of_feats, labels,
    ...
    # Iterate through architectures and train each one
    def plot_results(train_acc, val_acc, arch_num):
        plt.figure(figsize=(12, 6))
        plt.plot(train_acc)
        plt.plot(val_acc)
        plt.title('Model accuracy for Architecture {}'.format(arch_num))
        plt.ylabel('Accuracy')
        plt.ylim([0, 1.5])  # Set the limits of the y-axis here
        plt.xlabel('Epoch')
    ...
    for i, arch in enumerate(architectures):
        ...
        # Save the accuracies per epoch for both training and validation
        train_acc.append(history.history['accuracy'])
        val_acc.append(history.history['val_accuracy'])

        # Plot the results after each architecture is trained
        plot_results(history.history['accuracy'], history.history['val_accuracy'], i + 1)
